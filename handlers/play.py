...
async def playlist(client, message):
    ...
    if not queue:
        await message.reply_text('ഞാൻ ജീവനോടെ ഉണ്ട്')
    temp = []
    for t in queue:
        temp.append(t)
...
async def m_cb(b, cb):
    ...
    if type_ == 'pause':
        if (
            ...
            await cb.answer('enne connect cheyy', show_alert=True)
        else:
        ...
        if chat_id in callsmusic.pytgcalls.active_calls:
            ...
            callsmusic.pytgcalls.leave_group_call(chat_id)
            await cb.message.edit('ഞാൻ ഇറങ്ങി പോകും.....')
        else:
            await cb.answer('Chat is not connected!', show_alert=True)
